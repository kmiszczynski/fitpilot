...
import 'plank_test_screen.dart';

class ReverseSnowAngelsTestScreen extends StatefulWidget {
  final int squatCount;
  final String pushupType;
  final int pushupCount;

  const ReverseSnowAngelsTestScreen({
    super.key,
  ...
  @override
  State<ReverseSnowAngelsTestScreen> createState() =>
      _ReverseSnowAngelsTestScreenState();
}

class _ReverseSnowAngelsTestScreenState
    extends State<ReverseSnowAngelsTestScreen> {
  ...
  Future<void> _loadExerciseDetails() async {
    final result =
        await _exerciseRepository.getExerciseById('reverse_snow_angels');
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
              'Enter the number of reverse snow angels you completed:',
  ...
  Future<void> _saveCountAndContinue(int count) async {
    ...
    debugPrint('   Reverse Snow Angels: $count');
    ...
    final String message = count == 0
        ...
        : 'Great job! You did $count reverse snow angels 💪';
                  ...
                  children: [
                    ...
                    if (!_isTimerRunning && !_isTimerFinished) ...[
                      ...
                      _buildInstructionItem(
                          context, 'Lie face down on the floor'),
                      _buildInstructionItem(
                          context, 'Keep your forehead lightly on the ground'),
                      _buildInstructionItem(
                          context, 'Lift your arms slightly off the floor'),
                      _buildInstructionItem(context,
                          'Move them slowly from overhead to your sides and back'),
                      const SizedBox(height: AppConstants.spacingXLarge),

                      // Tips section
                      _buildSectionTitle(context, 'Tips:'),
                      const SizedBox(height: AppConstants.spacingMedium),
                      _buildTipCard(context, Icons.fit_screen,
                      ...
                      _buildTipCard(context, Icons.warning_amber_outlined,
                          'Stop if you feel discomfort in your shoulders or neck'),
                      const SizedBox(height: AppConstants.spacingXLarge),
